...
public class VvdParser {
    ...
    public static ParsedVvd parse(byte[] data) {
        ...
        try {
            ...
            LOGGER.debug("[VvdParser] Fixup parse error (non-fatal): {}", e.getMessage());
        }
        ...
        try {
            ...
            LOGGER.debug("[VvdParser] Vertex read error (non-fatal): {}", e.getMessage());
        ...
        try {
            ...
            LOGGER.debug("[VvdParser] Fixup apply error (non-fatal): {}", e.getMessage());
    ...
    private static List<StudioVertexExt> readRawVertices(ByteBuffer buf, ParsedVvd result, int bufferLimit) {
        ...
        for (int i = 0; i < numVertices; i++) {
            ...
            // VVD vertex layout (48 bytes): boneweights, position, normal, texcoord
            // mstudioboneweight_t: weight[3](12) + bone[3](3) + numbones(1)
    ...
    private static void applyFixups(ParsedVvd result, List<StudioVertexExt> rawVertices) {
        // Always store raw vertices - VTX origMeshVertID indices reference raw VVD vertices,
        // not fixup-reordered ones. The C++ parser also uses raw vertices for mesh building.
        ...
        // Build separate LOD-specific vertex lists for LOD selection
        int maxLOD = 4;
