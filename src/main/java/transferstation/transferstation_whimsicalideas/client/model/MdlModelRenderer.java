...
public class MdlModelRenderer {
    ...
    public static void render(LivingEntity entity, PoseStack poseStack, MultiBufferSource bufferSource,
        ...
        if (JavaModelRenderer.hasModel()) {
            // Attempt to render Java model with LOD support
            try {
                SourceModelData modelData = JavaModelRenderer.getModelData();
                if (modelData != null) {
                    // Try to render with LOD 0 (base mesh)
                    SourceModelData lodData = modelData.getMeshesForLod(0);
                    if (!lodData.meshes.isEmpty()) {
                        // Temporarily replace meshes in current model to render LOD 0
                        JavaModelRenderer.setModelData(lodData);
                        JavaModelRenderer.renderModel(entity, poseStack, bufferSource, packedLight, partialTicks);
                        return;
                    }
                }
            } catch (Exception e) {
                LOGGER.error("[MdlModelRenderer] Error rendering Java model with LOD", e);
            }
