...
public class MdlParser {
    ...
    private static final int TEXTURE_ENTRY_SIZE = 24;
    ...
    public static ParsedModel parse(byte[] data) {
        ...
        for (StudioMesh mesh : result.meshes) {
            try {
                ...
                System.out.println("[MdlParser] Mesh vertex data not in MDL (expected for VVD-based models): " + e.getMessage());
            }
        }
    ...
    private static StudioHdr2 parseStudioHdr2(ByteBuffer buf, StudioHeader header, int bufferLimit) {
        ...
        try {
            ...
            System.out.println("[MdlParser] StudioHDR2 parse error (non-fatal): " + e.getMessage());
        }
    ...
    private static void safeRun(SafeRunnable r, String section) {
        try {
            ...
            String msg = "[MdlParser] Non-fatal error parsing " + section + ": " + e.getMessage();
    ...
    private static StudioHeader parseHeader(ByteBuffer buf, int bufferLimit) {
        assertInBounds(buf.position(), 348, bufferLimit, "header");
    ...
    private static void parseModels(ByteBuffer buf, ParsedModel result, int bufferLimit) {
        int totalModels = 0;
        for (StudioBodyPart bp : result.bodyParts) {
    ...
    private static void parseTextures(ByteBuffer buf, ParsedModel result, int bufferLimit) {
        ...
        assertInBounds(textureIndex, numTextures * TEXTURE_ENTRY_SIZE, bufferLimit, "textureindex");

        for (int i = 0; i < numTextures; i++) {
            int entryOff = textureIndex + i * TEXTURE_ENTRY_SIZE;
            ...
            if (nameOff > 0) {
                int absNameOff = textureIndex + nameOff;
    ...
    private static void parseCdTextures(ByteBuffer buf, ParsedModel result, int bufferLimit) {
        ...
        for (int i = 0; i < numCd; i++) {
            ...
            if (nameOff > 0) {
                int absNameOff = cdIndex + nameOff;
    ...
    private static String readNullTerminatedString(ByteBuffer buf, int offset, int bufferLimit) {
        ...
        byte b;
        while (sb.length() < 256 && buf.position() < bufferLimit && (b = buf.get()) != 0) {
            sb.append((char) b);
        }
        buf.position(savedPos);
        return sb.toString();
