...
MdlParser::ParsedMdl MdlParser::parse(const std::vector<uint8_t>& data) {
    ...
    for (int i = 0; i < numBodyParts; i++) {
        StudioBodyPart bp;
        readAt(result.header.bodypartindex + i * BODYPART_SIZE, &bp, BODYPART_SIZE);
    ...
    if (result.header.numtextures > 0 && result.header.textureindex > 0) {
        ...
        for (int i = 0; i < numTextures; i++) {
            int entryOff = result.header.textureindex + i * TEXTURE_ENTRY_SIZE;
            StudioTexture tex;
            readAt(entryOff, &tex, TEXTURE_ENTRY_SIZE);

            std::string texName;
            if (tex.nameOffset > 0) {
                int nameOff = result.header.textureindex + tex.nameOffset;
                texName = readFixedString(raw, nameOff, 64);
            }
            result.textures.push_back(tex);
            // NOTE: Do NOT push texture names into cdTextures!
            // cdTextures are a separate list from the cdtextureindex section below.
        }
    }
