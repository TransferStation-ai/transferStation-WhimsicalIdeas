...
constexpr int FORMAT_DXT1 = 0;
constexpr int FORMAT_DXT3 = 1;
constexpr int FORMAT_DXT5 = 2;
constexpr int FORMAT_BGRA8888 = 12;
constexpr int FORMAT_RGB888 = 5;
constexpr int FORMAT_BGR888 = 6;
constexpr int FORMAT_RGBA8888 = 21;
...
VtfDecoder::DecodedTexture VtfDecoder::decode(const std::vector<uint8_t>& data) {
    ...
    auto getBlockSize = [](int fmt) -> int {
        switch (fmt) {
            case FORMAT_DXT1: return 8;
            case FORMAT_DXT3:
            case FORMAT_DXT5: return 16;
            default: return 4;
        }
    };
    ...
    size_t dataSize = static_cast<size_t>(rowBlocks) * colBlocks * blockSize;
    ...
    switch (imageFormat) {
        case FORMAT_DXT1: decodeDXT1(rawData.data(), width, height, result.rgbaData); break;
        case FORMAT_DXT3: decodeDXT3(rawData.data(), width, height, result.rgbaData); break;
        case FORMAT_DXT5: decodeDXT5(rawData.data(), width, height, result.rgbaData); break;
        default: decodeRawRGBA(rawData.data(), width, height, result.rgbaData, blockSize); break;
    }
    ...
    for (int by = 0; by < blockH; by++) {
        for (int bx = 0; bx < blockW; bx++) {
            ...
            if (c0 > c1) {
                ...
                colors[2] = ((static_cast<uint32_t>(r0) + r1) / 2 << 16) |
                            ((static_cast<uint32_t>(g0) + g1) / 2 << 8) |
                            ((static_cast<uint32_t>(b0) + b1) / 2) | 0xFF000000;
...
void VtfDecoder::decodeRawRGBA(const uint8_t* data, int width, int height,
                                std::vector<uint8_t>& rgba, int bytesPerPixel) {
    int srcOffset = 0;
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            int dstOff = (y * width + x) * 4;
            if (srcOffset + bytesPerPixel <= static_cast<int>(rgba.size())) {
                uint8_t b = data[srcOffset++];
                uint8_t g = data[srcOffset++];
                uint8_t r = data[srcOffset++];
                uint8_t a = (bytesPerPixel >= 4) ? data[srcOffset++] : 255;
                rgba[dstOff + 0] = r;
                rgba[dstOff + 1] = g;
                rgba[dstOff + 2] = b;
                rgba[dstOff + 3] = a;
