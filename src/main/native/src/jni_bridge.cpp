    ...
    JNIEnv* env, jclass, jlong handle)
{
    ...
    auto freeMeshes = [](std::vector<MeshData>& meshes) {
        for (auto& mesh : meshes) {
            ...
            if (mesh.textureId) GlRenderer::destroyTexture(mesh.textureId);
        }
    };
    freeMeshes(it->second->meshes);
    freeMeshes(it->second->lodMeshes1);
    freeMeshes(it->second->lodMeshes2);
    freeMeshes(it->second->lodMeshes3);
